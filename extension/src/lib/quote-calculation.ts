...
export interface QuotePlanInput {
  ...
  insurance: { amountUsdTotal: string; source: string } | null;
...
export function calculateQuote(input: QuoteInput, now = Date.now()): CalculatedPlan[] {
  ...
  const result: CalculatedPlan[] = input.plans.map(p => {
    ...
    if (p.propulsion === 'phev' || p.propulsion === 'unknown') throw new Error('动力分类及DG适用性须确认后核算');
    let insurance = 0n;
    if (p.insurance != null) { string(p.insurance.source, '保险来源'); insurance = decimal(p.insurance.amountUsdTotal); }
    const base = decimal(p.vehicle.amount) * q;
    const cost = (p.vehicle.currency === 'CNY' ? toUsd(base) : base) + ocean + dg + (ground ? toUsd(ground) : 0n) + insurance;
    let total = cost;
...
Include one <quote_input>JSON</quote_input> block at the END of [Full Translation & Strategy], or an internal discussion. Leave WhatsApp Reply empty for this intermediate calculation step. CRM will compute and send results back automatically in this same conversation, then you will compose the complete final reply. Do not claim it is already calculated/saved.
