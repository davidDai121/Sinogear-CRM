...
import { renderSalesWorkMemory, loadSalesWorkMemory } from './sales-work-memory';
...
export async function runDueFollowup(db: SupabaseClient<Database>, run: (opts: GptRunOptions) => Promise<GptRunResult>, state: FollowupRunnerState, now = Date.now()) {
  ...
  for (const task of ordered) {
    ...
    try {
      ...
      const [knowledge, memory] = await Promise.all([
        loadGptApprovedKnowledge(db, template.id, task.org_id), loadSalesWorkMemory(db, task.org_id, task.contact_id),
      ]);
